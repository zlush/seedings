...
type Searchable = { title: string; campana: string; marca: string };

// Busca en creador (@usuario o teléfono), campaña y marca.
export function matchesQuery(item: Searchable, query: string): boolean {
  const q = fold(query).trim();
  if (!q) return true;

  const haystack = fold([item.title, item.campana, item.marca].join(" "));
